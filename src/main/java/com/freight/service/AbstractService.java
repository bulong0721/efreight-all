...
public abstract class AbstractService {
	...
	protected <T> PageResult<T> search(Class<T> entityClass,
	...
	public <T> List<T> selectByExpr(Class<T> entityClazz, ADExpression expr,
	...
	public <T> List<T> selectByQuery(Class<T> entityClazz, String query,
	 ...
	 * @return
	 */
	protected <T> List<T> selectListByNamedQuery(String queryName,
			Class<T> clazz) {
	 ...
	 */
	protected <T> List<T> selectListByNamedQuery(String queryName,
			Class<T> clazz, Map<String, Object> paramMap) {
	 ...
	 */
	protected <T> List<T> selectListByNamedQuery(String queryName,
			Class<T> clazz, Map<String, Object> paramMap, int offset, int limit) {
		try {
			TypedQuery<T> query = em.createNamedQuery(queryName, clazz);
			if (null != paramMap) {
				for (Entry<String, Object> pEntry : paramMap.entrySet()) {
	...
	static class ServiceUtil {
		...
		public static Predicate buildWhere(CriteriaBuilder cb, Root<?> root,
				ADExpression expr) {
			if (null == expr) {
				return null;
			}
			if (expr.isParent()) {
				...
				List<Predicate> predList = new ArrayList<Predicate>(pred
						.getExpressions().size());
				...
				case Between:
					return cb.between(target,
							StringUtil.toString(expr.getValue1()),
							StringUtil.toString(expr.getValue2()));
				case NotBetween:
					return cb.not(cb.between(target,
							StringUtil.toString(expr.getValue1()),
							StringUtil.toString(expr.getValue2())));
				case Like:
					return cb.like(target,
							StringUtil.toString(expr.getValue1()));
				case NotLike:
					return cb.notLike(target,
							StringUtil.toString(expr.getValue1()));
				default:
					break;
				}
		...
		public static void toString(ADExpression expr, StringBuffer buffer,
				int level) {
			if (null == expr) {
				return;
			}
			if (expr.isParent()) {
				ADPredicate pred = (ADPredicate) expr;
				ServiceUtil.appendTab(buffer, level)
						.append(pred.getBooleanOperator().getSymbol())
						.append(" ");
				for (ADExpression subExpr : pred.getExpressions()) {
				...
				buffer.append(expr.getColumnName()).append(" ")
						.append(expr.getFieldOperator().getSymbol())
						.append(" ").append(expr.getValue1());
