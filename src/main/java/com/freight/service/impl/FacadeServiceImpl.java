...
public class FacadeServiceImpl extends AbstractService implements FacadeService {
    ...
    static {
        ...
        tableMap.put(117, CMoveV.class/* 发车管理 */);
    ...
    @Override
    public PageResult<?> getAllRefList() {
        List<ADReference> references = selectListByNamedQuery("queryAllRef", ADReference.class);
        List<LookupModel> resultList = new ArrayList<>();
        if (null != references) {
            for (ADReference reference : references) {
                resultList.add(new LookupModel(reference));
            }
        }
    ...
    @Override
    public PageResult<?> lookup4Table(Map<String, String> paramMap, Integer refValueID) {
        ...
        Map<String, Object> argMap = toMap("filter", '%' + rawValue + '%');
        List<?> rows = selectListByNamedQuery(refTable.getQueryName(), entityClass, argMap, offset, limit);
        PageResult<?> result = new PageResult<>(rows, rows.size());
        if (metaDemand) {
            List<ADColumn> columns =
                selectListByNamedQuery("queryLookupColumnsByTable", ADColumn.class,
    ...
    public boolean saveOrder(String entityText) {
        ...
        em.flush();
        if (!order.isPickup()) {
            invService.inbound(order, orderLine);
        }
        return false;
    }
    ...
    public PageResult<?> searchMoveline(Map<String, String> paramMap) {
        ...
        List<?> rows = selectListByNamedQuery("queryMoveline", CMoveLineV.class, queryMap, offset, limit);
        return new PageResult<>(rows, rows.size());
    }
